        ...
        public async Task<IActionResult> CreateFilme([FromServices] IOptions<ConnectionStringOptions> options, Filme filme)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(options.Value.MyConnectionLTA))
                {
                    ...
                    command.CommandText = @"insert into Filmes (title, description, gender, releaseDate, classification, rate) 
                                            values
                                            (@title, @description, @gender, @releaseDate, @classification, @rate)";
